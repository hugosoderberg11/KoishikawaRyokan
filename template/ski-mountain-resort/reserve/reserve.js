...
function applyI18n() {
  $$('[data-i18n]').forEach((el) => {
    ...
    else if (val.includes('<br')) el.innerHTML = val;
  ...
  $('#footer-site-name') && ($('#footer-site-name').textContent = pick(siteData.name));

  const tel = siteData.telephone;
  const telRaw = siteData.telephoneRaw;
  $$('#sidebar-tel').forEach((el) => {
    el.textContent = tel;
...
function updateCheckoutDate() {
  ...
  if (checkin && nights > 0) checkoutEl.value = addDays(checkin, nights);
  else checkoutEl.value = '';
  updateSummary();
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form?.addEventListener('submit', (e) => {
    ...
    setTimeout(() => {
      ...
      submitBtn.style.background = 'var(--sky-deep)';
