...
const LANG_KEY = 'kaiseki-ryokan-lang';
    ...
    return `
      <article class="room-card fade-in">
        <div class="room-card__img">
          <img src="${r.image}" alt="${pick(r.name)}" width="800" height="1000" loading="lazy" />
        ...
        <div class="room-card__body">
          ...
          <a href="reserve/?room=${r.id}" class="btn btn--gold">${t('room.btn')}</a>
        </div>
      </article>`;
  }).join('');
  initScrollReveal();
  ...
  el.innerHTML = plans.map(p => {
    ...
    <article class="plan-card${p.featured ? ' plan-card--featured' : ''} fade-in">
      ...
      <div class="plan-card__body">
        ...
        <a href="reserve/?plan=${p.id}" class="btn ${p.featured ? 'btn--gold' : 'btn--outline-light'}">${t('plan.btn')}</a>
      </div>
    </article>`;
  }).join('');
  initScrollReveal();
...
function renderCuisine(dishes) {
  const el = $('#cms-cuisine');
  if (!el || !dishes?.length) return;
  el.innerHTML = dishes.map(d => `
    <article class="dish-card fade-in">
      <div class="dish-card__img">
        <img src="${d.image}" alt="${pick(d.name)}" width="600" height="750" loading="lazy" />
        <button class="lightbox-trigger" type="button" data-group="cuisine" data-src="${d.image}" data-caption="${pick(d.name)}" aria-label="${pick(d.name)}を拡大表示"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7"/></svg></button>
      </div>
      <div class="dish-card__body">
        <span class="dish-card__cat">${pick(d.category)}</span>
        <h3>${pick(d.name)}</h3>
