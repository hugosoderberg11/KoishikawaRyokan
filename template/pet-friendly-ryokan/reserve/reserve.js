...
const LANG_KEY = 'pet-ryokan-lang';
...
function applyI18n() {
  ...
  $$('#header-tel, #sidebar-tel, .phone-number').forEach((el) => {
    el.textContent = tel;
...
function updatePetSummary() {
  const petType = $('#b-pet-type')?.value;
  const petCount = $('#b-pet-count')?.value;
  const petDetail = $('#b-pet-detail')?.value?.trim();

  if (!petType || !petCount) {
    $('#summary-pet') && ($('#summary-pet').textContent = '—');
    return;
  }

  const typeLabel = getSelectedLabel($('#b-pet-type'));
  const parts = [`${typeLabel} × ${petCount}`];
  if (petDetail) parts.push(petDetail);
  $('#summary-pet') && ($('#summary-pet').textContent = parts.join(' / '));
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('#b-pet-detail')?.addEventListener('input', updatePetSummary);
  ...
  form?.addEventListener('submit', (e) => {
    ...
    setTimeout(() => {
      ...
      submitBtn.style.background = 'var(--green-deep)';
